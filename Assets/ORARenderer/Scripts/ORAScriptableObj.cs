namespace ORARenderer
{
	...
	public class ORAScriptableObj : MonoBehaviour
	{
		private const string MAIN_DIRECTORY = "Assets/ORARenderer/";
		private const string OUTPUT_SUB_DIR = "Extracted/";

		[SerializeField] Root arcadianImage;
		[SerializeField] Image arcadianthing;
		...
		private void ReadXml(string directory)
		{
			...
			arcadianImage = JsonConvert.DeserializeObject<Root>(json);
			arcadianthing = arcadianImage.image;
			Debug.Log(arcadianthing.h);

			File.WriteAllText(directory + "/output.json", json);
